...
description: What you need to know about large Azure Virtual Machine Scale Sets to use them in your application.
author: mimckitt
ms.author: mimckitt
...
- Fault domains and upgrade domains are only consistent within a placement group. This architecture does not change the overall availability of a scale set, as VMs are evenly distributed across distinct physical hardware, but it does mean that if you need to guarantee two VMs are on different hardware, make sure they are in different fault domains in the same placement group. For more information, see [Availability options](../virtual-machines/availability.md). 
