...
You must specify the fault domain count for the Virtual Machine Scale Set. For regional (nonzonal) deployments, Virtual Machine Scale Sets offers the same fault domain guarantees as availability sets. However, you can scale up to 1000 instances. For zone-spreading scale sets, where you're spreading instances across multiple availability zones, you must use the *max spreading* approach (the fault domain count must be set to 1).
