...
> [!IMPORTANT]
> Previously, the process of deploying container groups on virtual networks used [network profiles](./container-instances-virtual-network-concepts.md#network-profiles) for configuration. However, network profiles are retired as of the `2021-07-01` API version. Use the latest API version, which relies on [subnet IDs](/azure/virtual-network/subnet-delegation-overview) instead.
