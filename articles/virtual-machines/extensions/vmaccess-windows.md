...
[Set-AzVMAccessExtension](/powershell/module/az.compute/set-azvmaccessextension) 
...
[Set-AzVMExtension](/powershell/module/az.compute/set-azvmextension)
...
| Error message: 'Cannot update Remote Desktop Connection settings for Administrator account. Error: System.Runtime.InteropServices.COMException (0x800706D9)' | VMAccess enables Remote Desktop access by updating Windows RDP configuration and associated firewall rules. This error typically occurs when the **Windows Defender Firewall** (`mpssvc`) service is disabled or not running. Set the service startup type to **Automatic**, start the service, make sure any required dependent services are running, and then retry the extension operation. |
