...
### Azure Boost and NVMe VM benefits
...
Azure Boost also optimizes performance by using the industry-standard NVMe interface. This interface capitalizes on the low latency and internal parallelism of solid-state storage drives. For details on the performance that Azure Boost offers, refer to the [Microsoft blog post about general availability of Azure Boost](https://techcommunity.microsoft.com/t5/azure-infrastructure-blog/announcing-the-general-availability-of-azure-boost/ba-p/3981384).

## SCSI to NVMe

For v6, Ebsv5, and later VMs, you need to transition from SCSI to NVMe. For more information, read about [converting SCSI to NVMe by using Linux](../../articles/virtual-machines/nvme-linux.md).

## Supportability

### VM sizes

Typically, the older generations of general-purpose, memory-optimized, and compute-optimized VMs (D/Ev5 or Fv2 and older) support SCSI. The newer generations (Ebsv5, Da/Ea/Fav6 and newer) typically support only the NVMe storage interface.

Some offerings support both the SCSI and NVMe interfaces. Ebsv5 is an example of such an offering.

Starting from Da/Ea/Fav6, we introduced the NVMe interface for local SSD disks.
...
If you think your workload will benefit from NVMe performance, you must switch to the Generation 2 VMs. To create a Generation 2 VM, follow the [documented steps](/azure/virtual-machines/generation-2#creating-a-generation-2-vm).

> [!NOTE]
> We currently don't support NVMe disks on Generation 1 VMs.

If you attempt to use a Generation 1 VM image, you won't be able to select an NVMe-capable VM. Or you'll receive an error message that says the selected image isn't supported for NVMe.

#### Availability of support

NVMe support is available in more than 50 of the most popular OS images. We're not adding NVMe support to older OS images. However, we continuously improve the OS image coverage. For updates on the latest OS image support for both Linux and Windows, we recommend [this documentation on OS images that support NVMe](/azure/virtual-machines/enable-nvme-interface).

If the OS that you want to use doesn't support NVMe, use a VM series that still has SCSI support. Many of the latest Azure VM generations are NVMe-only and require an OS image that supports NVMe.
...
You can create an VM enabled with the NVMe interface only by using an image marked as NVMe. The image must be available in [Microsoft Marketplace](https://ms.portal.azure.com/#view/Microsoft_Azure_Marketplace/MarketplaceOffersBlade/selectedMenuItemId/home) or shared in [Azure Compute Gallery](/azure/virtual-machines/shared-image-galleries?tabs=azure-cli) within your company.

> [!WARNING]
> If you use an unmarked image, a SCSI-based VM is created. The VM might not perform as you intended.
...
:::image type="content" source="./media/enable-nvme/nvme-faq-2.png" alt-text="Screenshot that shows an example of choosing an operating system image that isn't supported for NVMe.":::

### Use the Azure portal to enable the NVMe interface during VM creation
