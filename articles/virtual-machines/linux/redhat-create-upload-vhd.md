...
In this article, you learn how to prepare a Red Hat Enterprise Linux (RHEL) virtual machine (VM) for use in Azure. The versions of RHEL that are covered in this article are 7.x, 8.X., 9.x. This article covers preparation steps for the following hypervisors: Hyper-V, Kernel-based Virtual Machine (KVM), VMware, and Kickstart.
...
<details>
<summary> RHEL 7 using Hyper-V Manager</summary>
...
<details>
<summary>RHEL 8+/9+ using Hyper-V Manager</summary>
...
<details>
<summary>RHEL 7 using KVM</summary>
### RHEL 7  using KVM
This section shows you how to use KVM to prepare a [](#rhel-6-using-kvm) or [](#rhel-7-using-kvm) distro to upload to Azure.

1. Download the RHEL 7 KVM image of  from the Red Hat website. This procedure uses  as an example.
...
<details>
<summary>RHEL 8+/9+ using KVM</summary>
### RHEL 8+ and 9+ using KVM
1. Ensure that the Network Manager service starts at boot time:
...
<details>
<summary>RHEL 7 using VMware</summary>
### RHEL 7 using VMware
1. Create or edit the `/etc/sysconfig/network` file, and add the following text:
...
<details>
<summary>RHEL 8+/9+ using VMware</summary>
### RHEL 8+ and 9+ using VMware
...
<details>
<summary>RHEL 7 using Kickstart</summary>
### RHEL 7 from a kickstart file
This section shows you how to prepare a  distro from an ISO by using a kickstart file.
...
<details>
<summary>RHEL 8+/9+ using Kickstart</summary>
### RHEL 8+ and 9+ using Kickstart
1. Ensure that the Network Manager service starts at boot time:
...
1. Convert the qcow2 image to the VHD format.
    ...
    gawk 'match($0, /"virtual-size": ([0-9]+),/, val) {print val[1]}')
    rounded_size=$((($size/$MB + 1)*$MB))
    sudo qemu-img resize rhel-8.raw $rounded_size
    ```
