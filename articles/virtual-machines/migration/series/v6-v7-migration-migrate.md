...
```mermaid
flowchart TD
  A["Identify source VM"] --> HB{"VM hibernated?"}
  HB -->|Yes| HR["Resume, let it settle, shut down to Stop (deallocated)"]
  HR --> B{"Starting point?"}
  HB -->|No| B
  B -->|Greenfield| G["Deploy v6/v7 from a current Gen 2, NVMe, MANA-ready image"]
  B -->|"Gen 1 (v2/v3)"| C1["Plan Gen 1 to Gen 2 upgrade or use a Gen 2 image"]
  B -->|"Gen 2 (v4/v5)"| C2["Confirm a Gen 2 image"]
  C1 --> D{"Image NVMe and MANA ready?"}
  C2 --> D
  D -->|No| D1["Rebuild the image: Gen 2, NVMe, and MANA"]
  D1 --> E{"Already running on NVMe?"}
  D -->|Yes| E
  E -->|Yes| F1["Near ready: go straight to validation"]
  E -->|No| F{"App keeps data on the OS disk or uses hard-coded device paths?"}
  F -->|Yes| H["Back up and keep a bootable clone on a compatible older SCSI size"]
  F -->|No| I["Snapshot or back up the VM"]
  H --> J{"Migration method?"}
  I --> J
  J -->|"Most cases: redeploy"| K1["Deploy a new v6/v7 VM from the updated image"]
  J -->|"In place: keep the OS disk"| K2["Run the SCSI to NVMe conversion"]
  K1 --> L["Validate: boot, NVMe disks, MANA, drivers, application"]
  K2 --> L
  F1 --> L
  G --> L
  L --> M{"Validation passes?"}
  M -->|No| R["Roll back to the clone or backup, fix, then retry"]
  R --> J
  M -->|Yes| N{"Data left on the old OS disk?"}
  N -->|Yes| O["Copy OS-disk data from the clone to the new v6/v7 VM"]
  N -->|No| P["Workload owner sign-off"]
  O --> P
  P --> Q["Optimize cost and performance, then retire the old VM and clone"]
```

A few notes on the flow:

- **Keep a clone, not just a snapshot, when the OS disk holds data.** Cross-generation moves deploy from an updated image, so the new VM starts on a fresh OS disk (see [Persistent application data on the OS disk](v6-v7-migration-plan.md#persistent-application-data-on-the-os-disk)). The in-place [conversion](v6-v7-migration-nvme-conversion.md) keeps the existing OS disk but changes the device paths the OS sees. Either way, a bootable clone on a compatible older (SCSI) size gives you a clean rollback and a running copy you can read the OS-disk data from.
...
4. Validate boot, disk discovery, and connectivity (see [Validate and optimize](v6-v7-migration-validate.md)).
...
1. Score workloads (ready now, small refresh, or plan the region or family) and pick a pilot. See [Assess readiness](v6-v7-migration-assess.md).
...
For the full set of design considerations, see [Plan a workload migration](v6-v7-migration-plan.md).
...
- Group VMs by application dependency; sequence non-production before production.
- Keep waves small enough to validate within the window.
- For platform fleets, add a new pool or model and shift after validation; keep the old one until confirmed.
- Reconfirm capacity in the target region and zone at the start of each wave.
...
- Platform validation passes (boot, disk, network).
- The workload owner confirms the application is healthy.
- The rollback window closes with approval, and the migration record is updated.

## Phase 5: Close and expand

1. Capture the migrated count and scope; document any one-time remediations.
1. Review cost and performance, and rightsize where headroom exists. See [Validate and optimize](v6-v7-migration-validate.md).
...
- [Convert a VM from SCSI to NVMe in place](v6-v7-migration-nvme-conversion.md)
- [Validate and optimize](v6-v7-migration-validate.md)
