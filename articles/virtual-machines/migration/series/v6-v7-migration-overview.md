...
:::image type="content" source="media/v6-v7-migration-journey.png" alt-text="Diagram showing v2 through v5 VMs and their images moving through a platform modernization step to validated v6 and v7 VMs.":::
...
1. **[Assess](v6-migration-assess.md).** Use a short assessment to separate ready-now candidates from those needing image, driver, path, or capacity remediation.
1. **[Plan](v6-migration-plan.md).** Work through the considerations for Generation 2 boot, NVMe storage, MANA networking, Azure Boost offload, region, zone, capacity, and images.
1. **[Migrate](v6-migration-migrate.md).** Use a wave model that proves the platform pattern once, then scales through controlled rings.
1. **[Validate and optimize](v6-migration-validate.md).** Keep validation focused on boot, disks, drivers, networking, and workload-owner sign-off, then optimize for cost and performance.
