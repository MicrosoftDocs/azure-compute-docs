...
This article answers common questions about moving workloads to the v6 and v7 Azure VM series. For the full planning guidance, see [Plan a workload migration](v6-migration-plan.md).
...
1. Generation 2 (UEFI) readiness.
1. An NVMe-ready OS and image.
1. A MANA-ready OS and drivers.
1. Disk-path remediation (replace hard-coded SCSI paths).
1. A temporary-disk dependency review.
1. Target region and zone availability.
1. Backup and rollback readiness.

For the full list, see [Assess readiness](v6-migration-assess.md).
...
Plan and test rollback before migration. Because you deploy alongside the existing instances, rollback is usually "keep the old VM or pool until the new one is validated, then retire it." The in-place conversion can also switch the controller back from NVMe to SCSI, however this approach is not recommended for Production. Either way, keep a documented plan with a backup or snapshot, an owner, a maintenance window, and decision criteria.

## What tooling is involved?

For non-production workloads, for in-place moves, a community script switches a VM's controller from SCSI to NVMe, see [Convert a VM from SCSI to NVMe in place](v6-migration-nvme-conversion.md). Tooling automates checks and conversion steps, but it doesn't replace workload-dependency assessment or application validation.
...
- **AKS:** add a new v6/v7 node pool, validate workloads, then cordon and drain old nodes.
...
Multi-size strategies can improve capacity flexibility, but they must be validated. Confirm that the workload tolerates the selected CPU families, memory ratios, disk limits, networking limits, and performance differences. For AKS, use separate node pools with labels and taints. For Azure Virtual Desktop, separate host pools or rings can simplify the user experience. For tightly coupled stateful tiers, avoid multi-size designs unless the application has been tested across the full allowed set.

## How should marketplace or ISV appliances be handled?

Confirm vendor support before production migration. For products such as firewalls, monitoring tools, endpoint security, backup, or packet-capture appliances, validate the exact version, image, licensing model, NIC count, accelerated networking, throughput, failover, logging, and support path. If the product is deployed from Azure Marketplace, confirm the publisher image supports the target VM family; if it's directly procured, confirm support with the vendor. See [ISV network and storage appliances](v6-migration-plan.md#isv-network-and-storage-appliances). The migration of an ISV appliance is usually achieved by redeploying new from the Marketplace.
...
For the estate-scale approach, see [Sequence and automate estate-scale migrations](v6-migration-plan.md#sequence-and-automate-estate-scale-migrations).

## Next steps

- [Assess readiness for the v6 and v7 series](v6-migration-assess.md)
- [Plan a workload migration](v6-migration-plan.md)
