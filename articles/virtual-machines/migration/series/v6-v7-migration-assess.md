...
For new deployments, there's nothing to remediate. Choose the v6 or v7 series intentionally and start clean:
...
Greenfield deployments can skip the remediation guidance that follows and go straight to sizing and capacity in [Plan the migration](v6-v7-migration-plan.md).
...
Most of these signals are one-time and easy to remediate.
...
For fleets that deploy VMs for you, the move is a new pool or image version, not a per-VM resize, and validation stays focused on the platform layer.

| Pattern | What to do | What to confirm (platform only) |
| --- | --- | --- |
| Azure Virtual Desktop host pools | Build a v6/v7 golden image, add canary hosts, drain old hosts, and expand by ring. | Image boots, sign-in works, profiles attach, and agents are healthy. |
...
> [!NOTE]
> Pod disruption budgets, ingress rules, autoscale thresholds, and application health probes are unchanged by the VM size, so you don't need to revalidate them as part of this move.
...
- Sponsor and workload owner confirmed.
- Business driver documented (capacity, performance, cost, or modernization).
- Source family identified (v2/v3/v4/v5) and deployment type (migration or greenfield).
- Production and non-production separated.
- Migration window and success/rollback criteria agreed.

**Compute and capacity**

- Current size and target v6/v7 size selected.
- VM generation confirmed (Generation 2 path planned if the source is Generation 1).
- Target region and zone requirement confirmed.
- Quota and availability confirmed per wave; [capacity reservation](/azure/virtual-machines/capacity-reservation-overview) considered.
- Fallback size or region noted.

**OS and image**

- OS version supported; image is Generation 2, NVMe, and MANA ready.
- Custom gallery images rebuilt and validated once; boot diagnostics enabled for the pilot.
- Image build and versioning via [Azure Image Builder](/azure/virtual-machines/image-builder-overview) and [Azure Compute Gallery](/azure/virtual-machines/azure-compute-gallery) for repeatability.

**Storage**

- OS and data disks inventoried; disk-path references checked for hard-coded SCSI paths.
- Temporary and local disk use reviewed; page file, `tempdb`, and scratch placement confirmed.
- Backup or snapshot taken before cutover; confirm [Azure Backup support](/azure/backup/backup-support-matrix-iaas) for Generation 2 and Trusted Launch.

**Networking**

- MANA OS and driver readiness confirmed.
- For network appliances: NIC count, IP forwarding, and vendor support for the family confirmed.
...
- Family-scoped reservations and savings plans replanned so discount coverage follows the workload.

## Exit criteria

- Workloads scored as ready now, ready after a small refresh, or plan the region or family.
- Target family, region, zone, and quota/capacity plan documented.
- Any small remediation assigned.
- Pilot (or greenfield first deployment) selected.
- Rollback approach noted, and workload owners approve the first wave.

## Next steps

- [Plan a workload migration to the v6 and v7 series](v6-migration-plan.md)
