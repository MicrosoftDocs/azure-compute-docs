---
title: Frequently asked questions about modernization to v6 and v7 VM series
...
# Frequently asked questions about modernization to v6 and v7 VM series
...
That depends on who owns the image and what you replace. The Assess and Plan articles cover image and VM-level remediation, so they apply only when you own the image. The Modernize runbook replaces individual VMs, so it applies only when that's your unit of replacement.
...
No, treat it as a platform modernization. The target VM can change the boot mode, storage interface, networking, image requirements, and regional or zonal availability. Validate readiness first, then modernize in controlled waves so boot, storage, networking, application behavior, monitoring, and rollback are all covered.
...
## What if I run Dv3, Dsv3, Ev3, or Esv3 VMs?
...
Validate and update custom images before modernization. A custom image that works on older VM generations might not be prepared for NVMe or the v6/v7 deployment requirements. Standardize on [Azure Image Builder](/azure/virtual-machines/image-builder-overview) and [Azure Compute Gallery](/azure/virtual-machines/azure-compute-gallery) so you make the fix once and reuse it.

## What if a workload depends on `/dev/disk/azure/scsi1/lunX` paths?

Those paths don't exist after the move to NVMe-based disk presentation. Identify the dependencies and replace them with stable identifiers (by-UUID references or `/dev/disk/azure/*` symlinks) before modernization.

## What if the workload uses SQL Server tempdb on the local temporary disk?

Don't assume the same temporary-disk behavior on v6 and v7. The local disk exists only on `d`-suffixed sizes, is presented as NVMe, and is re-created raw on every stop/deallocate cycle. Review `tempdb` placement, performance requirements, and persistence expectations, and move or redesign the dependency before modernization if needed.

## What if security or antivirus software uses filter drivers?

Review those agents before modernization. Filter drivers, especially on Windows, can cause stability or boot issues when the storage interface changes, and Secure Boot requires signed drivers. Confirm a current, signed version and validate in a pilot before production.
...
For a modernization cutover, the question is often moot because you're deallocating anyway. Quiesce the application, shut it down to Stop (deallocated), and then take the restore point. Whichever route you take, restore the backup in a test before the wave instead of only confirming that the backup exists.
...
- Group by application dependency and modernization pattern.
