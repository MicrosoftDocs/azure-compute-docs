---
title: Modernize to the v6 and v7 VM series
description: Overview of modernizing Azure VM workloads from the v2 through v5 series to the v6 and v7 series—what changes, effort by starting point, target families, and the modernization journey.
author: rod-reis
ms.author: rosanto
...
# Modernize to the v6 and v7 VM series
...
This modernization playbook is for architects and infrastructure teams planning a move to the v6 or v7 series, for a single workload or across a large estate.
...
The modernization effort depends on how the workload deploys, stores state, and recovers from the replacement of an individual VM. Because the prerequisites apply per image, not per VM, workloads built from a shared, validated image modernize by replacing the pool, while customer-managed applications, clustered databases, and virtual appliances need application-aware replication, configuration migration, or vendor certification.
