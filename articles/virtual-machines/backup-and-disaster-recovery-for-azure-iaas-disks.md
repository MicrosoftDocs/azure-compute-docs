...
## Azure managed disk built-in redundancy options
...
## Backup and disaster recovery scenarios for Azure managed disks
...
### Scenario 1: Protect major databases on Azure VMs
...
### Scenario 2: Protect a redundant Azure VM cluster
...
### Scenario 3: Back up an IaaS application on Azure VMs
...
### Scenario 4: Recover from IaaS application data corruption
...
### Compare Azure managed disk protection options
...
|Solution |Azure managed disk snapshots |Azure VM restore points |Azure Disk Backup or Azure VM backup|Azure Site Recovery|
|----------|-----------|------------|------------|------------|
|**Description**|Snapshot is a read-only point-in-time copy of the disk that you can use for backup. |Use Azure VM restore points to implement granular backup of all disks attached to your virtual machine.|Azure Disk Backup and Azure VM backup are fully managed Azure services that back up your data and help you recover it.|Azure Site Recovery helps ensure your organization's business continuity by keeping apps and workloads running during outages.|
|**Incremental Backup**|Yes   |Yes   |Yes   |Yes   |
|**Cross-Region Copy**|Yes   |Available in preview   |Yes, with Azure VM backup   |Yes   |
...
Use [Azure VM restore points](virtual-machines-create-restore-points.md) to set up granular backup and retention policies for all disks attached to your virtual machine. An individual VM restore point is a resource that stores the VM configuration and point-in-time application-consistent snapshots of all the managed disks attached to the VM. By using Azure VM restore points, you can easily capture multi-disk consistent backups of all disks attached to your VM.

Azure VM restore points have three levels of hierarchy – VM restore point collection, VM restore points, and disk restore points:
...
-	Level 2: Each VM restore point contains a disk restore point for each of the attached disks.
-	Level 3: A disk restore point consists of a snapshot of an individual managed disk.

Azure VM restore points are incremental. The first restore point stores a full copy of all disks attached to the VM, while each successive restore point contains only incremental changes to VM disks. By using Azure VM restore points, you can:
...
### Azure Disk Backup and Azure VM backup
...
### Other backup options for SQL Server on Azure VMs
...
## Next steps for Azure managed disk protection
