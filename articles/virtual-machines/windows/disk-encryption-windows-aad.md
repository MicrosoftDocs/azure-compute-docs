---
title: Enable Azure Disk Encryption with Microsoft Entra ID for Windows VMs (previous release)
...
# Customer intent: As a security administrator, I want to implement disk encryption on Windows IaaS VMs by using Azure services, so that I can protect sensitive data and comply with security standards without Microsoft Entra application parameters.
---
...
1. On the [Resource Manager template](https://github.com/Azure/azure-quickstart-templates/tree/master/quickstarts/microsoft.compute/encrypt-create-new-vm-gallery-image), select **Deploy to Azure**.

1. Select the subscription, resource group, resource group location, parameters, legal terms, and agreement. Select **Purchase** to deploy a new IaaS VM where encryption is enabled.

1. After you deploy the template, verify the VM encryption status by using your preferred method:
   ...
   - Select the VM, then select **Disks** under the **Settings** heading to verify encryption status in the portal. In the chart under **Encryption**, you'll see if it's enabled.
...
### <a name="bkmk_RunningWinVMPSH"></a> Enable encryption on existing or running VMs by using Azure PowerShell
...
### <a name="bkmk_RunningWinVMCLI"></a> Enable encryption on existing or running VMs by using Azure CLI
...
1. Select the subscription, resource group, resource group location, parameters, legal terms, and agreement. Select **Purchase** to enable encryption on the existing or running IaaS VM.
...
### <a name="bkmk_VHDprePSH"> </a> Encrypt VMs with pre-encrypted VHDs by using Azure PowerShell
...
- **Disable disk encryption by using Azure PowerShell:** To disable the encryption, use the [Disable-Azure​RmVMDisk​Encryption](/powershell/module/az.compute/disable-azvmdiskencryption) cmdlet.
     ```azurepowershell-interactive
     Disable-AzVMDiskEncryption -ResourceGroupName 'MyVirtualMachineResourceGroup' -VMName 'MySecureVM'
   ```
...
- **Disable encryption with a Resource Manager template:**
  ...
  1. Select **Deploy to Azure** from the [Disable disk encryption on running Windows VM](https://github.com/Azure/azure-quickstart-templates/tree/master/quickstarts/microsoft.compute/decrypt-running-windows-vm) template.
