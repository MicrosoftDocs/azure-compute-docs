---
title: Create a key vault for ADE with Microsoft Entra ID
...
---
# Create a key vault for ADE with Microsoft Entra ID

[!INCLUDE [Azure Disk Encryption retirement notice](~/reusable-content/ce-skilling/azure/includes/security/azure-disk-encryption-retirement.md)]

**Applies to:** :heavy_check_mark: Windows VMs

**The new release of Azure Disk Encryption eliminates the requirement for providing a Microsoft Entra application parameter to enable VM disk encryption. With the new release, you no longer need to provide Microsoft Entra credentials during the enable encryption step. Encrypt all new VMs by using the new release without Microsoft Entra application parameters. For instructions to enable VM disk encryption by using the new release, see [Azure Disk Encryption](disk-encryption-overview.md). VMs that were already encrypted with Microsoft Entra application parameters are still supported. Continue to maintain those VMs with the Microsoft Entra syntax.**
...
Creating and configuring a key vault for use with Azure Disk Encryption with Microsoft Entra ID (previous release) involves four steps:

1. Create a key vault.
1. Set up a Microsoft Entra application and service principal.
1. Set the key vault access policy for the Microsoft Entra app.
1. Set key vault advanced access policies.

You can also generate or import a key encryption key (KEK).

See the main [Create and configure a key vault for Azure Disk Encryption](disk-encryption-key-vault.yml) article for steps on how to [Install tools and connect to Azure](disk-encryption-key-vault.yml#install-tools-and-connect-to-azure).
...
> [!WARNING]
>To ensure the encryption secrets don’t cross regional boundaries, Azure Disk Encryption requires the key vault and the VMs to be in the same region. Create and use a key vault in the same region as the VM that you want to encrypt.


### Create a key vault by using PowerShell

You can create a key vault by using Azure PowerShell and the [New-AzKeyVault](/powershell/module/az.keyvault/New-azKeyVault) cmdlet. For other Key Vault cmdlets, see [Az.KeyVault](/powershell/module/az.keyvault/).
...
1. Create a new key vault by using [New-AzKeyVault](/powershell/module/az.keyvault/New-azKeyVault)

      ```azurepowershell-interactive
     New-AzKeyVault -VaultName 'MySecureVault' -ResourceGroupName 'MyKeyVaultResourceGroup' -Location 'East US'
   ```

1. Note the **Vault name**, **Resource group name**, **Resource ID**, **Vault URI**, and the **Object ID** that are returned for later use when you encrypt the disks.


### Create a key vault by using Azure CLI
You can manage your key vault by using Azure CLI and the [az keyvault](/cli/azure/keyvault#commands) commands. To create a key vault, use [az keyvault create](/cli/azure/keyvault#az-keyvault-create).
...
1. Create a new key vault by using [az keyvault create](/cli/azure/keyvault#az-keyvault-create).

     ```azurecli-interactive
     az keyvault create --name "MySecureVault" --resource-group "MyKeyVaultResourceGroup" --location "East US"
   ```

1. Note the **Vault name** (name), **Resource group name**, **Resource ID** (ID), **Vault URI**, and the **Object ID** that are returned for use later.

### Create a key vault by using a Resource Manager template
...
1. Select the subscription, resource group, resource group location, key vault name, object ID, legal terms, and agreement, and then select **Purchase**.
...
### Set up a Microsoft Entra app and service principal by using Azure PowerShell
To run the following commands, get and use the [Azure PowerShell module](/powershell/azure/what-is-azure-powershell).

1. Use the [New-AzADApplication](/powershell/module/az.resources/new-azadapplication) PowerShell cmdlet to create a Microsoft Entra application. MyApplicationHomePage and the MyApplicationUri can be any values that you choose.
...
1. The `$azureAdApplication.ApplicationId` is the Microsoft Entra client ID and the $aadClientSecret is the client secret that you'll use later to enable Azure Disk Encryption. Safeguard the Microsoft Entra client secret appropriately. Running `$azureAdApplication.ApplicationId` shows you the application ID.


<a name='set-up-an-azure-ad-app-and-service-principal-with-azure-cli'></a>

### Set up a Microsoft Entra app and service principal by using Azure CLI

You can manage your service principals by using Azure CLI and the [az ad sp](/cli/azure/ad/sp) commands. For more information, see [Create an Azure service principal](/cli/azure/create-an-azure-service-principal-azure-cli).
...
1. Create a new service principal.
   ...
   ```
1. The appId returned is the Microsoft Entra client ID used in other commands. It's also the SPN you'll use for az keyvault set-policy. The password is the client secret that you use later to enable Azure Disk Encryption. Safeguard the Microsoft Entra client secret appropriately.
...
Use the steps from the [Use portal to create a Microsoft Entra application and service principal that can access resources](/azure/active-directory/develop/howto-create-service-principal-portal) article to create a Microsoft Entra application. Each of these steps takes you directly to the article section to complete.
...
1. [Create a Microsoft Entra application](/azure/active-directory/develop/howto-create-service-principal-portal#register-an-application-with-azure-ad-and-create-a-service-principal)
   - You can use any name and sign-on URL you would like when creating the application.
1. [Get the application ID and the authentication key](/azure/active-directory/develop/howto-create-service-principal-portal#sign-in-to-the-application).
   ...
   - The application ID is used later as the AadClientId for Set-AzVMDiskEncryptionExtension and as the ServicePrincipalName for Set-AzKeyVaultAccessPolicy.
...
### Set the key vault access policy for the Microsoft Entra app by using Azure PowerShell
...
1. Set the key vault access policy for the AD application by using PowerShell.
...
### Set the key vault access policy for the Microsoft Entra app by using Azure CLI
...
Give the service principal you created by using the Azure CLI access to get secrets and wrap keys with the following command:
...
### Set the key vault access policy for the Microsoft Entra app by using the portal

1. Open the resource group with your key vault.
1. Select your key vault, go to **Access policies**, then select **Add new**.
1. Under **Select principal**, search for the Microsoft Entra application you created and select it.
1. For **Key permissions**, check **Wrap Key** under **Cryptographic operations**.
1. For **Secret permissions**, check **Set** under **Secret management operations**.
1. Select **OK** to save the access policy.
...
The Azure platform needs access to the encryption keys or secrets in your key vault to make them available to the VM for booting and decrypting the volumes. Enable disk encryption on the key vault or deployments fail.

### Set key vault advanced access policies by using Azure PowerShell
...
### Set key vault advanced access policies by using the Azure CLI
...
1. Select the box labeled **Enable access to Azure Disk Encryption for volume encryption**.
1. Select **Enable access to Azure Virtual Machines for deployment** or **Enable access to Azure Resource Manager for template deployment**, if needed.
1. Select **Save**.
...
### Set up a key encryption key by using Azure PowerShell
Before you use the PowerShell script, review with the Azure Disk Encryption prerequisites to understand the steps in the script. The sample script might need changes for your environment. This script creates all Azure Disk Encryption prerequisites and encrypts an existing IaaS VM, wrapping the disk encryption key by using a key encryption key.
...
If you want to use certificate authentication, you can upload one to your key vault and deploy it to the client. Before you use the PowerShell script, review with the Azure Disk Encryption prerequisites to understand the steps in the script. The sample script might need changes for your environment.
...
If you want to use certificate authentication and wrap the encryption key with a KEK, you can use the below script as an example. Before you use the PowerShell script, review with all the previous Azure Disk Encryption prerequisites to understand the steps in the script. The sample script might need changes for your environment.
