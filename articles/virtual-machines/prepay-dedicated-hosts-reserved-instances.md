...
Purchasing a reservation for Azure Dedicated Hosts provides a billing discount only. It doesn't reserve or guarantee that Dedicated Host capacity will be available when you deploy. Capacity constraints can still block deployment even when quota is approved and a reservation is purchased. 
