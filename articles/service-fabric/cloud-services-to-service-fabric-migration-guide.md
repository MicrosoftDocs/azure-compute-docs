...
**Container Support Considerations:** Service Fabric Managed Clusters support Windows containers, similarly to traditional Service Fabric clusters. However, container runtimes such as Mirantis Container Runtime (formerly Docker Enterprise) are not included in the default node OS image and must be installed separately when containerized workloads are required. When planning a lift-and-shift migration, ensure that any container runtime dependencies and operational requirements are accounted for as part of the migration design. For additional guidance, see [Continuation of container support in Azure Service Fabric](https://github.com/Azure/Service-Fabric-Troubleshooting-Guides/blob/master/Deployment/Mirantis-Guidance.md). If your application requires IIS, Windows-specific server components, or other dependencies that would be best containerized, you need to use a traditional Service Fabric cluster with Windows Container support instead of a Managed Cluster. Consider this limitation carefully when planning your lift-and-shift migration approach.  Refer to [Containerize existing Windows applications](/previous-versions/azure/architecture/service-fabric/modernize-app-azure-service-fabric#containerize-existing-windows-applications)
