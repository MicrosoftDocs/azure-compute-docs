...
- Although there are [many metrics](/azure/azure-monitor/autoscale/autoscale-common-metrics) supported by auto-scale rules, it is still a limited set. If your scenario calls for scaling based on some metric not covered in that set, then auto-scale rules may not be a good option.
...
The process of scaling a node type up or down is different depending on whether it is a nonprimary or primary node type.
